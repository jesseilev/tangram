...
type Msg = Noop | NewTriangles (List Tri)
...
view model =
  let
    frameSize = 1024
    triangles = Svg.g [] (List.map viewTriangle model.triangles)
...
hullLocation scalevel tri =
  ...
  in
    tri.anchor |> Point2d.scaleAbout Point2d.origin (toFloat(2 ^ tri.scalevel))
